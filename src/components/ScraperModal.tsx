...
import { Link2, Loader2, Check, AlertCircle, BookOpen, ChevronRight } from 'lucide-react';
      ...
      >
        <div className="p-6 border-b border-stone-100 flex items-center justify-between bg-white sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold">سحب فصل من رابط</h3>
          </div>
          ...
          {step === 'input' ? (
            <form onSubmit={handleScrape} className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-bold text-stone-600">رابط الفصل (يدعم حالياً 69shuba ومواقع أخرى)</label>
                <div className="relative">
                  <input 
                    type="url" 
                    required
                    className="w-full p-4 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none pr-12"
                    placeholder="https://www.69shuba.cx/txt/..."
                    value={url}
                    onChange={(e) => setUrl(e.target.value)}
                  />
                  <Link2 className="absolute right-4 top-4 text-stone-400" size={20} />
                </div>
                <p className="text-xs text-stone-400">الصق رابط الفصل الصيني وسنقوم باستخراج النص لك تلقائياً.</p>
              </div>
              ...
              <button 
                ...
                className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 disabled:opacity-50"
              >
            ...
            <div className="space-y-6">
              <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                ...
                <h4 className="text-stone-800 font-bold">{scrapedData?.title}</h4>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-bold text-stone-600">اختر الرواية لإضافة الفصل إليها</label>
                <select 
                  className="w-full p-4 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none appearance-none"
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-stone-600">معاينة النص المسحوب</label>
                <div className="bg-stone-50 p-4 rounded-2xl border border-stone-200 max-h-60 overflow-y-auto text-sm text-stone-600 whitespace-pre-wrap leading-relaxed">
                  {scrapedData?.content}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-[2] bg-emerald-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 disabled:opacity-50"
                >
